                                ...
                                <div class="card-body d-flex align-items-center py-3">
                                    <div class="me-3">
                                        <span class="badge bg-primary rounded p-2">
                                            <i class="ti ti-wallet ti-sm text-white"></i>
                                        </span>
                                    </div>
                                    <div>
                                        <small class="text-muted">Net Wallet</small>
                                        <h4 id="net_wallet_balance" class="mb-0">0</h4>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body d-flex align-items-center py-3">
                                    <div class="me-3">
                                        <span class="badge bg-warning rounded p-2">
                                            <i class="ti ti-clock ti-sm text-white"></i>
                                        </span>
                                    </div>
                                    <div>
                                        <small class="text-muted">Pending</small>
                                        <h4 id="pending_requests" class="mb-0">0</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="card-body d-flex align-items-center py-3">
                                    <div class="me-3">
                                        <span class="badge bg-danger rounded p-2">
                                            <i class="ti ti-ban ti-sm text-white"></i>
                                        </span>
                                    </div>
                                    <div>
                                        <small class="text-muted">Rejections</small>
                                        <h4 id="rejections" class="mb-0">0</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card shadow-sm bg-light h-100">
                                <div class="card-body d-flex align-items-center py-3">
                                    ...
                                    <div>
                                        ...
                                        <h4 id="todays_revenue" class="mb-0">0</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="card-body d-flex align-items-center py-3">
                                    <div class="me-3">
                                        <span class="badge bg-info rounded p-2">
                                            <i class="ti ti-list-check ti-sm text-white"></i>
                                        </span>
                                    </div>
                                    <div>
                                        <small class="text-muted">Transactions</small>
                                        <h4 id="total_transactions" class="mb-0">0</h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-8 mb-4">
                            <div class="card p-3">
                                ...
                                <div id="paymentsChart" style="height:320px"></div>
                            </div>
                        </div>
                        <div class="col-lg-4 mb-4">
                            <div class="card p-3">
                                <h5>Top Approvers</h5>
                                <ul id="topApprovers" class="list-unstyled mb-0"></ul>
                            </div>
                        </div>
                    </div>
                        ...
                        (function() {
                            ...
                            async function fetchStats() {
                                ...
                                try {
                                    ...
                                    document.getElementById('todays_revenue').innerText = fmt(data.today_revenue);
                                    document.getElementById('total_transactions').innerText = data.total;

                                    // top approvers
                                    const ul = document.getElementById('topApprovers');
                                    ul.innerHTML = '';
                                    if ((data.top_approvers || []).length === 0) {
                                        ul.innerHTML = '<li class="text-muted">No data</li>';
                                    } else {
                                        data.top_approvers.forEach(function(row) {
                                            const li = document.createElement('li');
                                            li.className = 'mb-2';
                                            li.innerHTML =
                                                `<div class="d-flex justify-content-between"><div>${row.name}</div><div>${fmt(row.total)}</div></div>`;
                                            ul.appendChild(li);
                                        });
                                    }

                                    // chart
                                    const series = (data.series || []).map(s => s.total);
                                    const categories = (data.series || []).map(s => s.date);

                                    if (window._paymentsChart) {
                                        window._paymentsChart.updateOptions({
                                            xaxis: {
                                                categories
                                            }
                                        });
                                        window._paymentsChart.updateSeries([{
                                            name: 'Payments',
                                            data: series
                                        }]);
                                    } else {
                                        ...
                                        window._paymentsChart = new ApexCharts(document.querySelector('#paymentsChart'), options);
                                        window._paymentsChart.render();
                                    }
    ...
    <div class="card">
        ...
        <div class="card-body">
            <ul class="list-unstyled mb-0">
                ...
                <li class="mb-3 pb-1">
                    <div class="d-flex align-items-start">
                        <div class="badge bg-label-secondary p-2 me-3 rounded"><i class="ti ti-discount-2 ti-sm"></i>
                        </div>
                        <div class="d-flex justify-content-between w-100 flex-wrap gap-2">
                            <div class="me-2">
